...
!************************************************************************************************** 
     ...
     real*8 :: time, term1,term2,term3, dt, pi, vp, omega, k1, k2, k3
...
!                w1 = val_data(10);
            ...
            v1(iaz+2) = + 3.d0*pi*(dsin(pi*zs_loc(in)))**2.d0*dsin(2.d0*pi*xs_loc(in))*dsin(2.d0*pi*ys_loc(in));
             ...
             !PLANE-WAVE SOLUTION INITIAL CONDITIONS
!            u0(iaz+0) = 0;
!            u0(iaz+1) = 0;
...
!            if (zs_loc(in) .ge. 50) then
!              u0(iaz+2) = -dcos(k1*xs_loc(in)+k2*ys_loc(in)+k3*zs_loc(in) + omega*dt);;
!              u1(iaz+2) = -dcos(k1*xs_loc(in)+k2*ys_loc(in)+k3*zs_loc(in));
!              v1(iaz+2) = dsin(k1*xs_loc(in)+k2*ys_loc(in)+k3*zs_loc(in));
