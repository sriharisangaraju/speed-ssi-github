...
!> @brief Makes not-honoring technique. Mechanical properties given node by node.
     ...
     subroutine MAKE_MECH_PROP_CASE_035(rho, lambda, mu, gamma, qs, qp, & !outputs
     ...
     yref = -0.7825*xs + 5032308.5 ! reference line to identify sub basin 
	...
	vp30 = 2000
     ...
     if  (ys .le. yref) then 
      ...
      ! + MATERIAL INSIDE THE THESS BASIN 
		...
		gamma = (3.1415*(2/3))/QS       ! max freq of 2 Hz     
		  ...
		  ! + MATERIAL INSIDE THE MYGDONIAN BASIN 
           ! VS = 150 + 1.625 * (Depth)
           ! VP  = 1500 + 3.00 * (Depth)
           ! rho = 2075 + 0.4375 * (Depth) 

           VS = 200 +  15.0 * (Depth)**0.63 
           VP = 1500 +  32.8 * (Depth)**0.63
           rho = 2075 + 0.55 * (Depth)             
           lambda = rho * (VP**2 - 2*VS**2)                                                    
           mu = rho * VS**2                                          
           qs = 0.1*VS
...
!                 qs = 200.d0
     ...
     end subroutine MAKE_MECH_PROP_CASE_035                                   
