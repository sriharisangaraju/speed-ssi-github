...
!     Dimensioning vector 'num_node_sism'(nodes number along each faults) - begin
         ...
         do i = 1,nload_sism_el
            if (num_node_sism(i) .eq. 1 .and. nload_sism_el .eq. 1) then
                ...
                deallocate(sism_el_glo, dist_el_glo, posx_el_glo, posy_el_glo, posz_el_glo) 
                 ...
                 allocate(posx_el_glo(max_num_node_sism*mpi_np), posy_el_glo(max_num_node_sism*mpi_np))
                 allocate(posz_el_glo(max_num_node_sism*mpi_np))
                 
                 sism_el_glo = 0
                 dist_el_glo = 0.d0
                 posx_el_glo = 0.d0; posy_el_glo = 0.d0; posz_el_glo = 0.d0;
                 ...
                 call MPI_ALLGATHER(pos_sour_node_x(:,i), max_num_node_sism, SPEED_DOUBLE, &
                                    posx_el_glo, max_num_node_sism, SPEED_DOUBLE, mpi_comm, mpi_ierr)
                                    
                 call MPI_ALLGATHER(pos_sour_node_y(:,i), max_num_node_sism, SPEED_DOUBLE, &
                                    posy_el_glo, max_num_node_sism, SPEED_DOUBLE, mpi_comm, mpi_ierr)
                                    
                 call MPI_ALLGATHER(pos_sour_node_z(:,i), max_num_node_sism, SPEED_DOUBLE, &
                                    posz_el_glo, max_num_node_sism, SPEED_DOUBLE, mpi_comm, mpi_ierr)
                 ...
                 do k = 1, mpi_np*max_num_node_sism
                  ...
                  deallocate(posx_el_glo, posy_el_glo, posz_el_glo)
                  
                  !> UPDATE NUMBER OF NODES INSIDE THE SUBTRIANGULAR FAULT
                  num_node_sism(i) = j;
                  !>-------------------------------------------------------
 ...
 !            if (mpi_id.eq.0) write(*,'(A)')
              ...
              do k = 1, j
                 ...
                 if (mpi_id.eq.0) write(*,*)  'distance node-Hypo in [m] = ', dist_sour_node_sism(k,i)
              enddo     
              if (mpi_id.eq.0) write(*,'(A)')
